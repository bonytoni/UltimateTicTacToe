...
// TicTacToe.java is necessary for this file to function
public class UltimateTicTacToe {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Player " + player +
            ", please enter the number corresponding to the board that you want to start.");
        ...
        while (!checkGSWin()) {
            if (freebie == true) {
                System.out.println("Player " + player +
                    ", you can place anywhere on the board that is available.");
                boardNum= kb.nextInt();
                while (gameStatus[boardNum] != '-') {
                    System.out.println("Player " + gameStatus[boardNum] +
                        " has already won this board. Choose another board to place your " +
                        player);
                    boardNum= kb.nextInt();
                }
                freebie= false;
            ...
            System.out.println("Player " + player +
                ", please enter the number corresponding to the position that you want to place.");
            posNum= kb.nextInt();
            while (collection[boardNum].checkTaken(player, posNum)) {
                System.out
                    .println("The position you have entered is already taken. Please try again.");
                posNum= kb.nextInt();
            }
            collection[boardNum].setPosition(posNum, player);
            printBoardGame();
    ...
    public static void printBoardGame() {
        for (int i= 0; i < 9; i+= 3) {
            for (int j= 0; j < 9; j+= 3) {
                System.out.print(collection[i].getBoard()[j] + "|" +
                    collection[i].getBoard()[j + 1] + "|" + collection[i].getBoard()[j + 2]);
                System.out.print("    ");
                System.out.print(
                    collection[i + 1].getBoard()[j] + "|" + collection[i + 1].getBoard()[j + 1] +
                        "|" + collection[i + 1].getBoard()[j + 2]);
                System.out.print("    ");
                System.out.print(
                    collection[i + 2].getBoard()[j] + "|" + collection[i + 2].getBoard()[j + 1] +
                        "|" + collection[i + 2].getBoard()[j + 2]);
                System.out.println();
            }
            System.out.println();
        }
    ...
    public static boolean checkGSRows() {
        for (int i= 0; i < 9; i+= 3) {
            if (checkDash(gameStatus[i], gameStatus[i + 1], gameStatus[i + 2])) { return true; }
        }
        return false;
    }
    ...
    public static boolean checkGSColumns() {
        for (int i= 0; i < 3; i++ ) {
            if (checkDash(gameStatus[i], gameStatus[i + 3], gameStatus[i + 6])) { return true; }
        }
        return false;
    }

    public static boolean checkGSDiagonals() {
        return checkDash(gameStatus[0], gameStatus[4], gameStatus[8]) ||
            checkDash(gameStatus[2], gameStatus[4], gameStatus[6]);
    }
