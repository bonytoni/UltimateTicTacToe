...
public class TicTacToe{
  ...
  public boolean checkRows(){
    for (int i = 0; i < 9; i+=3){
      if (checkNoDashes(board[i], board[i+1], board[i+2])){
      return true;
      }
    }
    return false;
  ...
  public boolean checkColumns(){
    for (int i = 0; i < 3; i++){
      if (checkNoDashes(board[i], board[i+3], board[i+6])){
      return true;
      }
    }
    return false;
  ...
  public boolean checkDiagonals(){
    return (checkNoDashes(board[0], board[4], board[8]) || checkNoDashes(board[2], board[4], board[6]));
  }
